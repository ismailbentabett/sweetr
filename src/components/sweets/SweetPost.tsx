        ...
        <div class='flex-it flex-row'>
          ...
          <article class='flex-it flex-grow flex-shrink cursor-pointer'>
              ...
              <div class='flex-it justify-between flex-row w-full'>
                <div>
                  ...
                  <span class='text-gray-400'>
                    {' '}
                    {format(new Date(sweet().created_at))}
                  </span>
                </div>
                <Show when={sweet().user!.id === user().data.id}>
                  <button
                    onClick={handleDelete}
                    class='text-gray-400 cursor-pointer transition hover:text-red-400'
                  >
                    <FiTrash size={16} />
                  </button>
                </Show>
            ...
            <div class='flex-it flex-row flex-grow-0 items-center mb-2'>
              <div class='flex-it mr-3 mb-3 w-full'>{sweet().content}</div>
            </div>
            <div class='flex-it flex-row flex-grow text-gray-400'>
              ...
              >
                ...
                {sweet().bookmarked && (
                  <BsBookmarkFill size={18} color='#D91346' />
                )}
              </button>
            </div>
          </article>
